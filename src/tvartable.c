...
int addTVarFromOperation(op ope, int tvar1, int tvar2){
    switch(ope){
        case add:
            ...
            write_int(7);write_int(0);write_int(tvar1);write_endl();
            write_int(7);write_int(1);write_int(tvar2);write_endl();
            write_int(1);write_int(0);write_int(0);write_int(1);write_endl();
            write_int(8);write_int(tvar1);write_int(0);write_endl();break;
        case sub:
            fprintf(f,"%%Load value from tvar %d and %d then sub and store in %d\n",tvar1,tvar2,tvar1);
            write_int(7);write_int(0);write_int(tvar1);write_endl();
            write_int(7);write_int(1);write_int(tvar2);write_endl();
            write_int(3);write_int(0);write_int(0);write_int(1);write_endl();
            write_int(8);write_int(tvar1);write_int(0);write_endl();break;
        case mul:
            fprintf(f,"%%Load value from tvar %d and %d then mul and store in %d\n",tvar1,tvar2,tvar1);
            write_int(7);write_int(0);write_int(tvar1);write_endl();
            write_int(7);write_int(1);write_int(tvar2);write_endl();
            write_int(2);write_int(0);write_int(0);write_int(1);write_endl();
            write_int(8);write_int(tvar1);write_int(0);write_endl();break;
        case divi:
            fprintf(f,"%%Load value from tvar %d and %d then div and store in %d\n",tvar1,tvar2,tvar1);
            write_int(7);write_int(0);write_int(tvar1);write_endl();
            write_int(7);write_int(1);write_int(tvar2);write_endl();
            write_int(4);write_int(0);write_int(0);write_int(1);write_endl();
            write_int(8);write_int(tvar1);write_int(0);write_endl();break;
        default:break;
    }
    delLastVal();
